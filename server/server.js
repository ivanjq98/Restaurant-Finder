require("dotenv").config(); // Declaration for env modules
const express = require("express");
const morgan = require("morgan");
const cors = require("cors");
const db = require("./db"); // immediately look the db folder
const app = express();
...
app.get("/api/v1/restaurants/:id", async (req, res) => {
  ...
  try {
    const restaurant = await db.query(
      ...
      [req.params.id]
    ); // req.params.id is $1 of the variable
...
app.listen(port, () => {
  console.log(`server is up and listening on port ${port}`); // ` ` backtick if you want to use ${variable} in the log
});
